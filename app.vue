...
<template>
  ...
  <div class="container px-6 mx-auto">
    ...
    <div class="flex flex-col">
      <div v-for="(section, index) in sections" :key="section.id" class="flex gap-3 py-2 flex-col lg:flex-row px-2 shadow-md lg:shadow-none mb-4 lg:mb-0 rounded-md lg:rounded-none"
        :class="section.id % 2 === 0 ? 'bg-base-200' : ''">
        <div class="flex gap-3 flex-col lg:flex-row">
          <div class="flex font-bold w-full lg:w-8 px-2 justify-center">{{ section.id }}.</div>
          <div class="flex items-center gap-3">
            <div class="flex w-44 text-white bg-secondary p-1 rounded-md">{{ section.from }}</div>
            <div class="flex w-44 bg-accent p-1 rounded-md">{{ section.to }}</div>
            <div class="flex text-gray-500 w-20 font-bold">{{ section.distance }} km</div>
          </div>
        </div>
        <div class="flex flex-col lg:flex-row gap-3">
          <div class="flex gap-3">
            ...
            <div class="flex gap-3">
              <input type="number" min="0" v-if="selectedRunners[index]" v-model="selectedRunners[index].pace.minutes"
                class="w-16 input input-sm input-bordered max-w-xs" />
          ...
          <div class="flex gap-3">
            ...
            <div v-if="selectedRunners[index]" class="flex">
              <div v-if="index === 0">
