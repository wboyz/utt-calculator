...
export const useSection = () => {
  ...
  const totalDistance = computed(() => {
    return sections.value.filter((section) => !!section.runner).reduce((acc, section) => acc + section.distance, 0).toFixed(0);
  });
  ...
  const totalPercent = computed(() => {
    return Math.floor((sections.value.filter((section) => !!section.runner).length / sections.value.length) * 100);
  });
